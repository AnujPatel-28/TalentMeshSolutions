...
import { JOBS, type Job } from './jobsData';
...
function toBrowseJob(job: Job) {
    const postedDays = parseInt(job.posted, 10) || 0;
    return {
        id: String(job.id),
        title: job.title,
...
const BROWSE_JOBS = JOBS.map(toBrowseJob);
...
export default function BrowseJobsPage() {
    ...
    const [jobs] = useState<any[]>(BROWSE_JOBS);
    ...
    const isSaved = useCallback((jobId: string) => guestSavedIds.has(jobId), [guestSavedIds]);
    ...
    const [selectedJobId, setSelectedJobId] = useState<string | null>(BROWSE_JOBS[0]?.id ?? null);
