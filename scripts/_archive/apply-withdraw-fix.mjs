...
async function run() {
  const query = `
    ...
    BEGIN
      ...
      -- Defensive internal actor validation (Security Hardening patch)
      ...
      -- D. Set transaction-local session variables for the trigger (dynamically called to bypass API gateway restrictions)
      ...
      -- F. Log recruiter/actor activity (fixed columns)
