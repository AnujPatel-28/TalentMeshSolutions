  ...
  return (
    ...
    >
      <button
        ...
        onClick={onToggle}
        className="flex w-full cursor-pointer select-none items-center justify-between gap-4 px-6 sm:px-7 py-5 text-left"
      >
        <span className="text-[clamp(1.1rem,1.5vw,1.3rem)] font-medium tracking-tight leading-snug text-slate-900 dark:text-slate-100">
          {item.question}
        </span>
        ...
        <motion.span
          ...
          className="inline-flex size-10 sm:size-12 shrink-0 items-center justify-center rounded-full bg-black/5 dark:bg-white/10 text-slate-900 dark:text-slate-100"
        >
          {isOpen ? (
            <svg
              width="14"
      ...
      >
        ...
        >
          <div className="text-base sm:text-lg leading-7 sm:leading-8 font-normal tracking-normal text-slate-600 dark:text-slate-300">
            {item.answer}
          </div>
        </motion.div>
      </motion.div>
...
export function MotionAccordion({
  ...
  return (
    <div className={cn("w-full", className)}>
      <div className="flex flex-col rounded-[34px] p-2 sm:p-3" style={{ gap }}>
        {items.map((item, i) => (
          <AccordionItem
            key={i}
            item={item}
